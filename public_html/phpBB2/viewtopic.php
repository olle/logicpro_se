 ...
 *
 *   $Id: viewtopic.php,v 1.3 2005/02/05 21:44:38 olle Exp $
 *
 *
 ***************************************************************************/
...
if ($is_auth['auth_edit']) {
	...
	$s_auth_can .= $lang['Rules_edit_cannot'] . '<br>';
}

if ($is_auth['auth_delete']) {
	$s_auth_can .= $lang['Rules_delete_can'] . '<br>';
} else {
	$s_auth_can .= $lang['Rules_delete_cannot'] . '<br>';
}

if ($is_auth['auth_vote']) {
	$s_auth_can .= $lang['Rules_vote_can'] . '<br>';
} else {
	$s_auth_can .= $lang['Rules_vote_cannot'] . '<br>';
}
...
if ($is_auth['auth_mod']) {
	...
	$lock_topic   = "modcp.$phpEx?" . POST_TOPIC_URL . "=$topic_id&amp;mode=lock&amp;sid=" . $userdata['session_id'];
	$unlock_topic = "modcp.$phpEx?" . POST_TOPIC_URL . "=$topic_id&amp;mode=unlock&amp;sid=" . $userdata['session_id'];
	$move_topic   = "modcp.$phpEx?" . POST_TOPIC_URL . "=$topic_id&amp;mode=move&amp;sid=" . $userdata['session_id'];
	$split_topic  = "modcp.$phpEx?" . POST_TOPIC_URL . "=$topic_id&amp;mode=split&amp;sid=" . $userdata['session_id'];
	$delete_topic = "modcp.$phpEx?" . POST_TOPIC_URL . "=$topic_id&amp;mode=delete&amp;sid=" . $userdata['session_id'];
}
...
if ( $can_watch_topic ) {
	if ( $is_watching_topic ) {
		$s_watching_topic = "<a href=\"viewtopic.$phpEx?" . POST_TOPIC_URL . "=$topic_id&amp;unwatch=topic&amp;start=$start&amp;sid=" . $userdata['session_id'] . '" class="submenu">' . $lang['Stop_watching_topic'] . '</a>';
	} else {
		$s_watching_topic = "<a href=\"viewtopic.$phpEx?" . POST_TOPIC_URL . "=$topic_id&amp;watch=topic&amp;start=$start&amp;sid=" . $userdata['session_id'] . '" class="submenu">' . $lang['Start_watching_topic'] . '</a>';
	}
}
...
$template->assign_vars(array(
	...
	'U_LOCK_TOPIC' => $lock_topic,
	'U_UNLOCK_TOPIC' => $unlock_topic,
	'U_MOVE_TOPIC' => $move_topic,
	'U_SPLIT_TOPIC' => $split_topic,
	'U_DELETE_TOPIC' => $delete_topic,
...
for ($i = 0; $i < $total_posts; $i++) {
	...
	if ($poster_id != ANONYMOUS) {
		$poster_profile_url = '<a href="' . append_sid("profile.$phpEx?mode=viewprofile&amp;" . POST_USERS_URL . "=$poster_id") .'" >';
		$poster_profile = $poster_profile_url . $postrow[$i]['username'] . '</a>';
	}
	
	$poster = ( $poster_id == ANONYMOUS ) ? $lang['Guest'] : $poster_profile;
	...
	$poster_joined = ( $postrow[$i]['user_id'] != ANONYMOUS ) ? $lang['Joined'] . ': ' . create_date('j/n Y', $postrow[$i]['user_regdate'], $board_config['board_timezone']) : '';
...
/////////////
/////// TODO: HANDLE THE ICONS FOR PM, ICQ etc.....
/////////////
	...
	$quote = '<a href="' . $temp_url . '" class="submenu">' . $lang['Reply_with_quote'] . '</a>';

	//////////////
	/////////
	////////
	///// TODO: KOLLA OM DET H�R SKA ANV�NDAS ?
	$temp_url = append_sid("search.$phpEx?search_author=" . urlencode($postrow[$i]['username']) . "&amp;showresults=posts");
	$search = '<a href="' . $temp_url . '" class="submenu">' . sprintf($lang['Search_user_posts'], $postrow[$i]['username']) . '</a>';
	//////////////////////////

	if (($userdata['user_id'] == $poster_id && $is_auth['auth_edit']) || $is_auth['auth_mod']) {
		$temp_url = append_sid("posting.$phpEx?mode=editpost&amp;" . POST_POST_URL . "=" . $postrow[$i]['post_id']);
		$edit = '<td class="btnLightToLightMiddle" width="9"></td>' .
				'<td class="btnLightMiddle">' .
				'<a href="' . $temp_url . '" class="submenu">' . $lang['Edit_delete_post'] . '</a>' .
				'</td>';
	} else {
		$edit = '';
	}
	...
	if ($is_auth['auth_mod'] ) {
		...
		$ip = '<td class="btnLightToLightMiddle" width="9"></td>' .
				'<td class="btnLightMiddle">' .
				'<a href="' . $temp_url . '" class="submenu">' . $lang['View_IP'] . '</a>' .
				'</td>';

		$temp_url = "posting.$phpEx?mode=delete&amp;" . POST_POST_URL . "=" . $postrow[$i]['post_id'] . "&amp;sid=" . $userdata['session_id'];
		$delpost = '<td class="btnLightToLightMiddle" width="9"></td>' .
				'<td class="btnLightMiddle">' .
				'<a href="' . $temp_url . '" class="submenu">' . $lang['Delete_post'] . '</a>' .
				'</td>';
	} else {
		$ip_img = '';
		$ip = '';
	...
	// Make any sig address clickable
	if ( $user_sig != '' ) {
