 ...
 *
 *   $Id: usercp_register.php,v 1.2 2005/01/30 18:42:42 olle Exp $
 *
 *
 ***************************************************************************/
...
if ( isset($HTTP_POST_VARS['submit']) ||
			...
			isset($HTTP_POST_VARS['cancelavatar']) || $mode == 'register' ) {
...
if ( isset($HTTP_POST_VARS['submit']) ) {
	...
	if ( $mode == 'editprofile' ) {
		...
		if (!preg_match("/mainstage*/i", $_POST['bonusq'])) {
		    $error = TRUE;
		    $error_msg .= ((isset($error_msg)) ? '<br />' : '' ) .
		      $lang['Bonus_question_wrong'];
		}
		// end mod

		if ( empty($username) || empty($new_password) || empty($password_confirm) || empty($email) || empty($location) || empty($signature)) {
			$error = TRUE;
 				...
 				else if ( $board_config['require_activation'] == USER_ACTIVATION_ADMIN )
 				{
 					...
 					while ($row = $db->sql_fetchrow($result))
 					{
 						...
 						$emailer->assign_vars(array(
 							...
 							// olle@studiomediatech.com 2007-07-21
							'U_FROM' => $location,
 							'U_WEBSITE' => $website,
 							'U_OCCUPATION' => $occupation,
 							'U_INTERESTS' => $interests,
 							'U_EMAIL' => $email,
 							'U_MSN' => $msn,
 							'U_ICQ' => $icq,
 							'U_AIM' => $aim,
 							...
 							// olle@studiomediatech.com 2007-07-21
 							'U_DELETE' => $server_url . '?mode=delete&' . POST_USERS_URL . '=' . $user_id . '&act_key=' . $user_actkey)
 						);
...
else
{
	...
	$template->assign_vars(array(
		...
		'L_BONUS_QUESTION'          => $lang['Bonus_question'],
		'L_BONUS_QUESTION_EXPLAIN'  => $lang['Bonus_question_explain'], 	
