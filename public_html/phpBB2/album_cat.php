	...
	for ($i = 0; $i < count($picrow); $i += $album_config['cols_per_page'])
	{
		...
		for ($j = $i; $j < ($i + $album_config['cols_per_page']); $j++)
		{
			...
			$template->assign_block_vars('picrow.pic_detail', array(
				...
				'RATING' => ($album_config['rate'] == 1) ? ( '<a href="'. append_sid("album_rate.$phpEx?pic_id=". $picrow[$j]['pic_id']) . '" class="submenu" >' . $lang['Rating'] . '</a>: ' . $picrow[$j]['rating'] . '<br />') : '',
				'COMMENTS' => ($album_config['comment'] == 1) ? ( '<a href="'. append_sid("album_comment.$phpEx?pic_id=". $picrow[$j]['pic_id']) . '" class="submenu" >' . $lang['Comments'] . '</a>: ' . $picrow[$j]['comments'] . '<br />') : '',
				'EDIT' => ( ( $auth_data['edit'] and ($picrow[$j]['pic_user_id'] == $userdata['user_id']) ) or ($auth_data['moderator'] and ($thiscat['cat_edit_level'] != ALBUM_ADMIN) ) or ($userdata['user_level'] == ADMIN) ) ? '<a href="'. append_sid("album_edit.$phpEx?pic_id=". $picrow[$j]['pic_id']) . '" class="submenu" >' . $lang['Edit_pic'] . '</a>' : '',
				'DELETE' => ( ( $auth_data['delete'] and ($picrow[$j]['pic_user_id'] == $userdata['user_id']) ) or ($auth_data['moderator'] and ($thiscat['cat_delete_level'] != ALBUM_ADMIN) ) or ($userdata['user_level'] == ADMIN) ) ? '<a href="'. append_sid("album_delete.$phpEx?pic_id=". $picrow[$j]['pic_id']) . '" class="submenu" >' . $lang['Delete_pic'] . '</a>' : '',
				'MOVE' => ($auth_data['moderator']) ? '<a href="'. append_sid("album_modcp.$phpEx?mode=move&amp;pic_id=". $picrow[$j]['pic_id']) .'" class="submenu" >F</a>' : '',
				'LOCK' => ($auth_data['moderator']) ? '<a href="'. append_sid("album_modcp.$phpEx?mode=". (($picrow[$j]['pic_lock'] == 0) ? 'lock' : 'unlock') ."&amp;pic_id=". $picrow[$j]['pic_id']) .'" class="submenu" >'. (($picrow[$j]['pic_lock'] == 0) ? 'L' : 'Ö') .'</a>' : ''
				)
			);

			// olle@studiomediatech.com 051016 - Added block switches for better page layout.
			
			if (($auth_data['edit'] and ($picrow[$j]['pic_user_id'] == $userdata['user_id']))
					or ($auth_data['moderator'] and ($thiscat['cat_edit_level'] != ALBUM_ADMIN))
					or ($userdata['user_level'] == ADMIN)) {
						
				$template->assign_block_vars('picrow.pic_detail.switch_edit', array());
			}
			
			if (($auth_data['delete'] and ($picrow[$j]['pic_user_id'] == $userdata['user_id']))
					or ($auth_data['moderator'] and ($thiscat['cat_delete_level'] != ALBUM_ADMIN))
					or ($userdata['user_level'] == ADMIN)) {
				
				$template->assign_block_vars('picrow.pic_detail.switch_delete', array());
			}
			
			if ($auth_data['moderator']) {
				
				$template->assign_block_vars('picrow.pic_detail.switch_move', array());
				$template->assign_block_vars('picrow.pic_detail.switch_lock', array());
			}
		}
	}

	$this_pagination = generate_pagination(append_sid("album_cat.$phpEx?cat_id=$cat_id&amp;sort_method=$sort_method&amp;sort_order=$sort_order"), $total_pics, $pics_per_page, $start);
	
	if (isset($this_pagination) and $this_pagination != '') {
		$template->assign_block_vars('switch_pagination', array());
	}

	$template->assign_vars(array(
		'PAGINATION' => $this_pagination,
